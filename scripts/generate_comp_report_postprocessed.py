...
def main():
    ...
    report_path1 = os.path.join(base_path, 'postprocessed_noun', 'report_structured')
                    ...
                    if tagger not in ['MatePosTagger', 'HepplePosTagger']:
                        if '_'.join(os.path.basename(file).split('_')[:-1]).lower() in os.path.basename(
                                ...
                                delta_pos_tags, dkpro_pos_tags, gold_pos_tags = generate_delta_pos_tags(dkpro_file, file,
                                                                                                        delta_file)
                                post_processed_delta_pos_tags = post_processed_generate_delta_pos_tags(delta_pos_tags, dkpro_file, dkpro_pos_tags,
                                                                       gold_pos_tags)
                                generate_classification_report(gold_pos_tags, post_processed_delta_pos_tags, dkpro_pos_tags,dkpro_report_path, delta_report_path)
                        ...
                        if '_'.join(os.path.basename(file).split('_')[:-1]).lower() in os.path.basename(
                            ...
                            post_processed_delta_pos_tags = post_processed_generate_delta_pos_tags(delta_pos_tags, dkpro_file, dkpro_pos_tags,
                                                                   gold_pos_tags)
                            generate_classification_report(gold_pos_tags, post_processed_delta_pos_tags, dkpro_pos_tags,
